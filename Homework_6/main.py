...
async def create_fake_users(count: int):
    ...
    return {'message': f'{count} тестовых пользователей создано'}
...
async def create_fake_products(count: int):
    for i in range(count):
        ...
        await db.database.execute(query)
    return {'message': f'{count} тестовых товаров создано'}
...
async def create_fake_orders(count: int):
    for i in range(count):
        ...
        await db.database.execute(query)
    return {'message': f'{count} тестовых заказов создано'}
...
async def update_product(product_id: int, new_product: models.ProductCreate):
    query = db.products.update().where(
        db.products.c.id == product_id).values(**new_product.dict())
    await db.database.execute(query)
...
async def update_order(order_id: int, new_order: models.OrderCreate):
    query = db.orders.update().where(
        db.orders.c.id == order_id).values(**new_order.dict())
    await db.database.execute(query)
